            ...
            <div style={{margin: '5px'}}>
                ...
                <button type="button" onClick={() => {
                    if(userid == null || userid == 'undefined'){
                        ...
                        fetch(`http://localhost/api/cart/insert?userid=${userid.key}&productCode=${productCode}&amount=${amount.current.value}`)
                        .then(() => { navigate('/cart/list');});
                    }
                }} className="btn btn-info">장바구니</button>&nbsp;
                <button type="button" onClick={() => {
