...
function WriteOrder(){
    ...
    const query = queryString.parse(params); // get 방식으로 전달된 쿼리스트링
    const route = query.route
    console.log('params ' + params)
    // JSON.stringify : Object → String 변환
    ...
    return (
        <>
        <h2>주문서 작성</h2>
        금액 : {money}, 배송료 : {delivery}, 총금액 : {totalMoney} <br />
        <table>
            <tbody>
                <tr>
                    <td>결제수단</td>
                    <td>
                        <input
                            ...
                            type="radio"
                            checked={method=="card"}
                            onChange={changeMethod} /> 카드
                        <input
                            value="account"
                            name="method"
                            type="radio"
                            checked={method=="account"}
                            onChange={changeMethod} /> 계좌이체
                    </td>
                </tr>
                <tr>
                    <td>카드번호</td>
                    <td><input ref={cardNumber} disabled={disableCard} /></td>
                </tr>
                <tr>
                    <td>우편번호</td>
                    <td><input ref={zipcode} /></td>
                </tr>
                <tr>
                    <td>주소</td>
                    <td><input ref={address1} /></td>
                </tr>
                <tr>
                    <td>상세주소</td>
                    <td><input ref={address2} /></td>
                </tr>
                <tr>
                    <td>전화</td>
                    <td><input ref={tel} /></td>
                ...
                <tr>
                    <td colSpan='2' align="center">
                        <button type="button" onClick={() => {
                            ...
                            fetch('http://localhost/api/order/insert', {
                                method: 'post', 
                                body : form
                            }).then(() => {
                                navigate('/order/list');
