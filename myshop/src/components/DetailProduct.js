...
function DetailProduct() {
    ...
    const {productCode} = useParams(); // APP.js 에서 전달한 파라미터
    const [data, loading] = useFetch('http://localhost/api/product/detail/' + productCode);
        ...
        return (
            ...
            <table>
                <tbody>
                    <tr>
                        <td>상품명</td>
                        <td><input ref={productName} defaultValue={data.dto.productName}/></td>
                    </tr>
                    <tr>
                        <td>가격</td>
                        <td><input ref={price} defaultValue={data.dto.price}/></td>
                    </tr>
                    <tr>
                        <td>상품설명</td>
                        <td><textarea rows='5' cols='60' ref={description} defaultValue={data.dto.description}/></td>
                    </tr>
                    <tr>
                        <td>상품이미지</td>
                        <td>
                                ...
                                <>
                                    ...
                                    <button type="button" onClick={() => {
                                        if(window.confirm('삭제할까요?')){
                                            fetch(`http://localhost/api/product/delete/${data.dto.productCode}`)
                                            .then(() => {navigate('/product/list');});
                                        }
                                    }} className="btn btn-">삭제</button>
                                    &nbsp;
                                </>
                                : ''
                            }
                            <button onClick={() => navigate('/product/list')} className="btn btn-info">목록</button>
                        </td>
                    </tr>
                </tbody>
            </table>
